...
class EffCSVFiller : public NtupleFillerBase {
            ...
            src_(iC.consumes<std::vector<T> >(iConfig.getParameter<edm::InputTag>("src"))) 
    {
        ...
        calib=BTagCalibration("CSVv2", std::string(std::getenv("CMSSW_BASE"))+"/src/MonoHTauTau/Configuration/data/CSVv2_Moriond17_B_H.csv");
            ...
            if( iEvent.getByToken(src_,handle)){
                for( unsigned int i=0; i<handle->at(0).jets().size(); i++){
                    ...
                    if (csv>0.8484){
                        ...
                        else if (jetflavor!=5) jf = BTagEntry::FLAV_UDSG; 
...
typedef EffCSVFiller<PATMuTrackPair> PATMuTrackPairEffCSVFiller;
