...
reco::Candidate::LorentzVector getGenBosonP4(const reco::GenParticleCollection& genParticles) {
	...
	if(genParticles.size()>0){
		for(reco::GenParticleCollection::const_iterator i = genParticles.begin();i!=genParticles.end();++i){ 
			int pdg = abs(i->pdgId());
			if ( ( i->fromHardProcessFinalState() &&  (pdg==11 || pdg==13 || pdg==12||pdg==14 ||pdg==16)   ) || i->statusFlags().isDirectHardProcessTauDecayProduct() ){ 
				p4 += i->p4();
			}
		}
	}
...
reco::Candidate::LorentzVector getGenBosonP4Vis(const reco::GenParticleCollection& genParticles) {
	...
	if(genParticles.size()>0){
		for(reco::GenParticleCollection::const_iterator i = genParticles.begin();i!=genParticles.end();++i){ 
			int pdg = abs(i->pdgId());
			if ( ( i->fromHardProcessFinalState() && (pdg==11 || pdg==13) ) || ( i->statusFlags().isDirectHardProcessTauDecayProduct() && !(pdg==12||pdg==14 ||pdg==16) )){ 
				p4Vis += i->p4();
			}
		}
	}
